...
Quest'ultima copre tutti i device immessi nel mercato europeo che scambiano dati tramite onde radio, tra cui prodotti collegati alla rete tramite Wi-Fi e Bluetooth, dispositivi wireless di #gloss[IoT] e altri, richiedendo che questi rispettino dei #gloss[requisiti di sicurezza] informatica.
...
La #gloss[verifica] manuale di questi Decision Trees è un'operazione delicata, tediosa e soggetta all'errore umano. Per cui, il progetto prevede lo sviluppo di una piattaforma interattiva che sia in grado di importare file strutturati relativi alle informazioni di un'attrezzatura specifica e del suo rispetto dei Decision Trees sopra descritti, di eseguirli automaticamente e di fornire all'utente una dashboard per visualizzare in maniera dettagliata l'adempimento dell'attrezzatura a ogni #gloss[requisito].

Sebbene il gruppo abbia uno spiccato interesse per il mondo dell'#gloss[IoT],
...
Inoltre, il gruppo trova troppo vaghe le specifiche tecnologiche richieste dal progetto, il quale non pone alcun vincolo né sul tipo di applicazione sviluppata (applicazione desktop o web), né sulla struttura dell'applicazione stessa, non richiedendo necessariamente di sviluppare una parte di #gloss[back-end].
...
Tra le funzionalità opzionali sono inoltre previste la ricerca semantica, la #gloss[verifica] delle firme digitali e l'accesso a pacchetti disponibili in #gloss[cloud].
...
Il progetto presenta un'architettura moderna, basata su microservizi e orientata al #gloss[cloud], con un'infrastruttura preferibilmente serverless su AWS. \
Tra le tecnologie suggerite figurano _Flutter_ per lo sviluppo multi-piattaforma e servizi #gloss[cloud] come AWS Lambda, DynamoDB, RDS e SageMaker per l'implementazione di componenti basate su Machine Learning. \
...
In particolare, l'azienda ha risposto a perplessità del gruppo relative principalmente all'utilizzo degli #gloss[LLM] per il trattamento dei dati sensibili degli utenti.

// NOTA [Elia]: aggiunti dubbi e considerazioni negative (dire più cose è meglio)
Nonostante #gloss[ci]ò, gran parte del gruppo non prova interesse verso lo sviluppo del front-end mobile e l'utilizzo delle tecnologie dell'AI generativa con Amazon SageMaker e Amazon Bedrock.\
Inoltre, è fortemente in disaccordo nell'utilizzo di un #gloss[LLM] come supporto per persone in situazioni delicate, il rischio che i componenti del gruppo temono è che una possibile allucinazione del modello, in situazioni così critiche, potrebbe portare a conseguenze tremende.
...
Il progetto proposto da _Zucchetti S.p.A._ nel capitolato *C6* prevede lo sviluppo di un editor di testo in formato _Markdown_, con l'integrazione di un #gloss[LLM] volto a migliorare o correggere il testo dell'utente. \
Tra i casi d'uso proposti per l'#gloss[LLM] vi sono la riscrittura del testo in registri di formalità differenti, la generazione di riassunti e la fornitura di critiche costruttive e suggerimenti di miglioramento da più punti di vista.\
...
Nonostante #gloss[ci]ò, dopo un'attenta votazione interna, il gruppo ha deciso di candidarsi per il capitolato C7, preferendolo per un margine molto ridotto rispetto a View4Life. Quest'ultimo rimane comunque considerato un ottimo progetto, apprezzato sia per la chiarezza della proposta sia per il valore formativo che può offrire.
