...
= Introduzione
Il presente verbale attesta che in data *9 dicembre 2025* si è svolto l'incontro con il proponente *M31 SRL*, in modalità remota tramite _Microsoft Teams_, dalle ore 14:30 alle 15:10.
Hanno partecipato:
- *da remoto*: Jaume Bernardi, Alessandro Dinato, Riccardo Graziani, Siria Salvalaio.

L'incontro aveva l'obiettivo di discutere le risposte fornite dal proponente ai dubbi precedentemente inviati via email dal gruppo.\

= Resoconto
== Discussione sui dubbi inviati al proponente

=== Anonimizzazione dei dati durante l'impersonificazione
Il proponente ha chiarito che quando un super-admin impersona un tenant-admin, l'accesso ai dati può essere regolato a livello di contratto: se il contratto prevede il permesso di impersonificazione allora il super-admin può visualizzare tutti i dati del tenant, altrimenti il tenant-admin è l'unico responsabile.
...
- Il *tenant-admin* visualizza una dashboard con i propri dati.
- Il *super-admin* visualizza una dashboard con dati aggregati e anonimizzati per ogni tenant, a meno che non abbia i permessi contrattuali per visualizzare i dati specifici di un tenant.

Il super-admin può vedere gli utenti creati dai tenant, ma eventuali limitazioni dipendono dai permessi contrattuali concessi per l'impersonificazione.
...
- creare, rimuovere o modificare un'associazione di un gateway a un tenant.
...
Per quanto riguarda il provisioning sicuro, il proponente ha indicato che:
- si può assumere che un gateway reale generi una coppia di chiavi pubblica/privata al primo avvio;
- il gateway stabilisce una comunicazione sicura con il cloud per scambiare tali chiavi;
- i certificati di autenticità sono installati da M31 nel cloud così da riconoscere i gateway autentici.
